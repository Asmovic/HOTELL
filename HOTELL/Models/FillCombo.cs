...
public class FillCombo
{
    ...
    public static void DropDownListItems_Services(int fieldindex, DropDownList cmbID, string mytable, string myfield, string mycode)
    {
        using (SqlConnection objConn = DBConnection.Connect())
        {
            using (SqlCommand sqlcmd = new SqlCommand())
            {
                ...
                cmbID.Items.Add("---Select From List---");
                sqlcmd.CommandText = "select * from " + mytable + " where " + myfield + "='" + mycode + "'";
                SqlDataAdapter das = new SqlDataAdapter(sqlcmd);
                ...
                foreach(DataRow dr in ds.Rows)
                {
                        ...
                        var mcode = dr["prod_serv_code"].ToString();
                        populate_prod_Name(cmbID, mcode);
                        //cmbID.Items.Add(dr.GetValue(fieldindex).ToString());
    ...
    public static void populate_prod_Name( DropDownList cmbID,  string mycode)
    {
        using (SqlConnection objConn = DBConnection.Connect())
        {
            using (SqlCommand sqlcmd = new SqlCommand())
            {
                ...
                sqlcmd.CommandText = "select * from Product_Service where Prod_Serv_Code ='" + mycode + "'";
                using (SqlDataReader dr = sqlcmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                      //  var mcode = dr["prod_serv_code"].ToString();
                        cmbID.Items.Add(dr["Prod_Serv_Desc"].ToString());
                    }
                }
            }
        }
